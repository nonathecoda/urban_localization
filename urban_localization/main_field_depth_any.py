...
class Localizer():
    ...
    def run(self, time_stamp):
        ...
        sampled_pose_estimates = sample_estimates_gauss(self.__scene, self.__camera, self.__guessed_pose, n=self.__config['sampling']['number_samples'], bounds = self.__scene.bounds, uncertainty=[self.__config['sampling']['sd_guess_x'],self.__config['sampling']['sd_guess_y'] ], pitch = -25)
        ...
        masked_depthmap_true_pose, masked_depthmap_estimate_pose = mask_depthmaps(self.__query_pose, self.__estimate_pose) 
...
if __name__ == "__main__":
    ...
    def initialisation(config, scene = None):
        ...
        camera = PinholeCamera.from_fov(width=531, height=708, hfov_deg=90)
        #### create query pose
        # load query images
        ...
        query_pose = PoseEstimate.create_from_depth_map(rgb, depth, camera, query_pose_hilla, name = 'query_pose', draw = False)
